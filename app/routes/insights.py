...
@router.get("/projects/{project_id}/experiments/{experiment_id}/gate")
async def experiment_gate(
    ...
    """Pass/fail quality gate for CI pipelines.

    Query params:
      - thresholds: comma list of metric:min pairs, e.g.
        ``faithfulness:0.7,refusal_accuracy:0.8``
      - min_overall: minimum mean across all metric averages
      - strict: when true, a failing gate returns HTTP 412 so shell scripts
        can gate on the status code (``curl -f``); otherwise always 200 with
        a ``pass`` field.

    Example:
      curl -f "$URL/api/projects/1/experiments/9/gate?thresholds=faithfulness:0.7&strict=true"
    """
    conn = db.init.get_db()
    exp = _require(
        conn,
    ...
    if exp["status"] != "completed":
        raise HTTPException(status_code=409, detail=f"Experiment is {exp['status']}, not completed")
    ...
    averages = {mn: round(totals[mn] / counts[mn], 4) for mn in totals if counts[mn]}

    checks: list[dict] = []
    for pair in (p.strip() for p in thresholds.split(",") if p.strip()):
        if ":" not in pair:
            raise HTTPException(status_code=400, detail=f"Invalid threshold '{pair}' — use metric:min")
        metric, _, raw_min = pair.partition(":")
        metric = metric.strip()
        try:
            min_value = float(raw_min)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=f"Invalid threshold value in '{pair}'") from exc
        actual = averages.get(metric)
        checks.append({
            "metric": metric,
            "min": min_value,
            "actual": actual,
            # A missing metric fails the gate — silence must not pass CI.
            "pass": actual is not None and actual >= min_value,
        })

    overall_mean = round(sum(averages.values()) / len(averages), 4) if averages else None
    if min_overall is not None:
        checks.append({
            "metric": "_overall",
            "min": min_overall,
            "actual": overall_mean,
            "pass": overall_mean is not None and overall_mean >= min_overall,
        })

    if not checks:
        raise HTTPException(status_code=400, detail="Provide thresholds and/or min_overall")
