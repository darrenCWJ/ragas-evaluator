...
const EMBEDDING_TYPES = ['dense_openai', 'dense_sentence_transformers', 'bm25_sparse'] as const;
...
export default function EmbeddingConfigPanel({ projectId, chunkConfigs, onConfigsChanged }: Props) {
  const [configs, setConfigs] = useState<EmbeddingConfig[]>([]);
  const [typeDefaults, setTypeDefaults] =
    useState<Record<EmbeddingType, string>>(FALLBACK_TYPE_DEFAULTS);
  ...
  const loadConfigs = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to load embedding configs');
    } finally {
      setLoading(false);
    }
  }, [projectId]);
  ...
  const isOpenAIEmb3 = embType === 'dense_openai' && modelName.startsWith('text-embedding-3');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-border bg-card/60 p-5">
        <h4 className="mb-4 text-sm font-semibold text-text-primary">New Embedding Config</h4>

        <label className="block">
          <span className="mb-1 block text-xs text-text-secondary">Name</span>
          <p className="mt-0.5 text-xs text-text-muted">
            A descriptive name for this embedding configuration
          </p>
          <input
            type="text"
            value={name}
        ...
        <label className="mt-3 block">
          ...
          <p className="mt-0.5 text-xs text-text-muted">
            dense_openai: OpenAI API embeddings (best quality) | dense_sentence_transformers: local
            model embeddings (free, slower) | bm25_sparse: keyword-based sparse embeddings (no ML)
          </p>
          <select
            value={embType}
        ...
        <label className="mt-3 block">
          <span className="mb-1 block text-xs text-text-secondary">Model Name</span>
          <p className="mt-0.5 text-xs text-text-muted">
            {embType === 'dense_openai' ? (
              <>
                OpenAI models:{' '}
                <code className="rounded bg-elevated px-1">text-embedding-3-small</code> (fast,
                cheap) | <code className="rounded bg-elevated px-1">text-embedding-3-large</code>{' '}
                (best quality, supports dimension reduction)
              </>
            ) : embType === 'dense_sentence_transformers' ? (
              <>
                Local models from{' '}
                <a
                  href="https://huggingface.co/models?pipeline_tag=sentence-similarity&sort=downloads"
                  target="_blank"
                ...
                : <code className="rounded bg-elevated px-1">all-MiniLM-L6-v2</code> (fast) |{' '}
                <code className="rounded bg-elevated px-1">all-mpnet-base-v2</code> (better quality)
                | <code className="rounded bg-elevated px-1">BAAI/bge-small-en-v1.5</code> |{' '}
                <code className="rounded bg-elevated px-1">nomic-ai/nomic-embed-text-v1.5</code>
              </>
            ) : (
        ...
        <label className="mt-3 block">
          <span className="mb-1 block text-xs text-text-secondary">Params (JSON, optional)</span>
          <p className="mt-0.5 text-xs text-text-muted">
            Optional JSON parameters for the embedding model
          </p>
          <textarea
            value={paramsJson}
            ...
            <label className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-text-muted">
                  OpenAI text-embedding-3 models support shortening the embedding dimensions. Lower
                  dimensions = faster search and less storage, with minimal quality loss. Default is
                  3072 for -large and 1536 for -small.
                </p>
              </div>
            </label>
            {useDimReduction && (
              <label className="mt-2 block">
                <span className="mb-1 block text-xs text-text-secondary">Dimensions</span>
                <select
                  value={dimensions}
      ...
      <div>
        <h4 className="mb-3 text-sm font-semibold text-text-primary">Saved Embedding Configs</h4>
                ...
                <li key={cfg.id} className="rounded-lg bg-card px-4 py-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex shrink-0 items-center gap-1">
                      ...
                      <button
                        onClick={() => setEmbedConfigId(embedConfigId === cfg.id ? null : cfg.id)}
                  ...
                  {expandedConfigId === cfg.id && (
                    <div className="mt-3 border-t border-border pt-3 space-y-1 text-xs text-text-muted">
                      ...
                      {cfg.model_name && (
                        <p>
                          <span className="text-text-secondary">Model:</span> {cfg.model_name}
                        </p>
                      )}
