...
export default function CsvUploadsList({ projectId, refreshKey }: Props) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5">
      <div className="mb-4">
        <h3 className="text-sm font-semibold text-text-primary">Uploaded CSVs</h3>
      ...
      {!loading && !error && csvConfigs.length > 0 && (
        <div className="space-y-2">
          {csvConfigs.map((config) => {
            ...
            const sourceFile = (config.config_json as Record<string, unknown>)?.source_file as
              | string
              | undefined;

            return (
              <div key={config.id} className="overflow-hidden rounded-lg border border-border">
                {/* CSV row header */}
                <button
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 text-xs text-text-muted">
                      {sourceFile && <span className="truncate">{sourceFile}</span>}
                      {sample && (
                        <span>
                          {sample.total} row{sample.total !== 1 ? 's' : ''}
                        </span>
                      )}
