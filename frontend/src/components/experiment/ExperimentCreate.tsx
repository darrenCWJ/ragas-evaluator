...
export default function ExperimentCreate({ projectId, onCreated }: Props) {
  ...
  if (loading) {
    return (
      <div className="py-6 text-center text-sm text-text-muted">Loading configurations...</div>
    );
  }

  // CSV bot configs auto-create their test set — don't require a pre-existing test set
  const noPrereqs =
    (testSets.length === 0 && !hasCsvBotConfigs) ||
    (ragConfigs.length === 0 && botConfigs.length === 0);
    ...
    <div>
      ...
      {noPrereqs && (
        <div className="rounded-lg border border-yellow-500/30 bg-yellow-500/10 px-4 py-3 text-sm text-yellow-300">
          ...
          {ragConfigs.length === 0 && <p>No RAG configs found. Complete the Build stage first.</p>}
        </div>
      )}
      ...
      {!noPrereqs && (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            />
            {touched && !nameValid && <p className="mt-1 text-xs text-red-400">Name is required</p>}
          </div>
          ...
          {mode === 'bot' && (
            <div>
              ...
              {touched && !botConfigValid && (
                <p className="mt-1 text-xs text-red-400">Bot connector is required</p>
              )}
          ...
          {mode === 'rag' && (
            <div>
              ...
              {touched && !ragConfigValid && (
                <p className="mt-1 text-xs text-red-400">RAG configuration is required</p>
              )}
              {ragConfigId !== '' &&
                (() => {
                  const selected = ragConfigs.find((rc) => rc.id === ragConfigId);
                  if (!selected || (!selected.chunk_config && !selected.embedding_config))
                    return null;
                  return (
                    <div className="mt-2 rounded-lg border border-border/50 bg-card/40 px-3 py-2 text-xs text-text-muted">
                      {selected.chunk_config && (
                        <p>
                          ...
                          {selected.chunk_config.name} ({selected.chunk_config.method})
                        </p>
                      )}
                      {selected.embedding_config && (
                        <p>
                          <span className="text-text-secondary">Embedding:</span>{' '}
                          {selected.embedding_config.name} ({selected.embedding_config.model_name})
                        </p>
                      )}
                      <p>
                        <span className="text-text-secondary">LLM:</span> {selected.llm_model} |
                        Search: {selected.search_type} | Top K: {selected.top_k}
                      </p>
                    </div>
                  );
                })()}
