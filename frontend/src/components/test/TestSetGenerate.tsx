...
export default function TestSetGenerate({ projectId, chunkConfigs, onTestSetCreated }: Props) {
  ...
  const handleAutoGeneratePersonas = async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : 'Failed to auto-generate personas');
    } finally {
  ...
  const handleSavePersonas = async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : 'Failed to save personas');
    } finally {
            ...
            {kgInfo?.exists && kgInfo.is_complete ? (
              <div className="rounded-lg border border-accent/20 bg-accent/5 px-3 py-2.5 text-xs">
                ...
                <p className="mt-0.5 text-text-muted">
                  Node texts will be used as the generation source. The stored KG is reused directly
                  — no rebuild.
                </p>
              </div>
            ) : kgInfo?.exists && !kgInfo.is_complete ? (
              <div className="rounded-lg border border-yellow-500/20 bg-yellow-500/5 px-3 py-2.5 text-xs text-yellow-400">
                Knowledge graph build is incomplete ({kgInfo.completed_steps}/{kgInfo.total_steps}{' '}
                steps). Finish building it before using it as a source.
              </div>
            ) : (
              <div className="rounded-lg border border-red-500/20 bg-red-500/5 px-3 py-2.5 text-xs text-red-400">
                No knowledge graph found for this project. Build one in the <strong>Build</strong>{' '}
                tab first.
              </div>
            )}
          </div>
          ...
          <div className="sm:col-span-2 rounded-lg border border-border bg-elevated/50 p-4 space-y-3">
            ...
            {kgBuilding ? (
              ...
              <div className="text-sm text-text-secondary space-y-1">
                ...
                <p className="text-xs text-text-muted">
                  ...
                  {kgInfo.is_complete === false &&
                    ` (partial — step ${kgInfo.completed_steps ?? 0}/11)`}
                </p>
                {kgInfo.is_complete === false && (
                    ...
                    <button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          setError(err instanceof ApiError ? err.message : 'Failed to reset KG');
                        }
                      }}
              ...
              <div className="space-y-3">
                <p className="text-xs text-text-muted">
                  Pre-build a knowledge graph for faster "Full" persona generation and richer test
                  sets.
                </p>
                ...
                <label className="flex cursor-pointer items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-[10px] leading-relaxed text-text-muted">
                      ...
                      <strong className="text-text-secondary">one LLM call per node</strong> instead
                      of 5 — roughly 3× faster. Slight quality trade-off.
                    </p>
                  </div>
                </label>
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      setKgBuilding(false);
                      setError(err instanceof ApiError ? err.message : 'Failed to start KG build');
                    }
                  }}
                  disabled={generating}
                        ...
                        {editingPersonaId === p.id ? (
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            >
                              ...
                              {p.question_style && (
                                <p className="truncate text-xs text-accent/70">
                                  {p.question_style}
                                </p>
                              )}
                            </button>
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const enabledKeys = QUESTION_CATEGORIES.map((c) => c.key).filter(
                            (k) => next[k],
                          );
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setCategoryDistribution((prev) => {
                              const enabledKeys = QUESTION_CATEGORIES.map((c) => c.key).filter(
                                (k) => enabledCategories[k] && k !== cat.key,
                              );
            ...
            {QUERY_TYPES.map((qt) => {
              ...
              return (
                <div key={qt.key} className="space-y-1.5">
                  <div className="flex items-baseline justify-between gap-2">
                    <div>
                      <label className="text-xs font-medium text-text-secondary">{qt.label}</label>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setQueryDistribution((prev) => {
                            const otherKeys = QUERY_TYPES.map((q) => q.key).filter(
                              (k) => k !== qt.key,
                            );
                            const remaining = 100 - raw;
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const raw = Number(e.target.value);
                      setQueryDistribution((prev) => {
                        const otherKeys = QUERY_TYPES.map((q) => q.key).filter((k) => k !== qt.key);
                        const remaining = 100 - raw;
                        const distributed = redistributeEvenly(otherKeys, remaining, prev);
                        return { [qt.key]: raw, ...distributed };
                      });
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const enabledKeys = GRAPH_RAG_CATEGORIES.map((c) => c.key).filter(
                            (k) => next[k],
                          );
                          if (enabledKeys.length > 0) {
                            const share = Math.round(100 / enabledKeys.length);
